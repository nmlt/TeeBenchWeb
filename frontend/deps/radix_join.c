...
void print_timing(uint64_t total, uint64_t build, uint64_t part,
    ...
    logger(INFO, "Total input tuples : %u", numtuples);
    logger(INFO, "Result tuples : %lu", result);
    logger(INFO, "Phase Total (cycles) : %lu", total);
    logger(INFO, "Phase Partition (cycles) : %lu", part);
    logger(INFO, "Phase Join (cycles) : %lu", build);
    logger(INFO, "Cycles-per-tuple           : %.4lf", cyclestuple);
    logger(INFO, "Cycles-per-tuple-partition : %.4lf", (double)part/(double)numtuples);
    logger(INFO, "Cycles-per-tuple-pass1     : %.4lf", (double)pass1/(double)numtuples);
    logger(INFO, "Cycles-per-tuple-pass2     : %.4lf", (double)pass2/(double)numtuples);
    logger(INFO, "Cycles-per-tuple-join      : %.4lf", (double)build/(double)numtuples);
...
//#ifdef NATIVE_COMPILATION
//            sprintf(buf, "%u,%u,%u,%u\n",
//                    Rtuples[hit-1].key, Rtuples[hit-1].payload, Stuples[i].key, Stuples[i].payload);
//#else
//            sprintf_s(buf, 512, "%u,%u,%u,%u\n",
//                      Rtuples[hit-1].key, Rtuples[hit-1].payload, Stuples[i].key, Stuples[i].payload);
//#endif
               ...
               bool materialize)
{
    ...
    const uint64_t numR  = R->num_tuples;
    uint32_t       Nhist = get_hist_size((uint32_t)numR);
...
prj_thread(void * param)
{
    ...
    const int thresh1 = (int)(MAX((1<<D), (1<<R)) * THRESHOLD1((unsigned long)args->nthreads));
...
#ifdef ITT_NOTIFS
        ocall_itt_event_start(1);
        ocall_itt_event_start(3);
#endif
#ifdef PCM_COUNT
        ocall_set_system_counter_state("Partition - 1st pass");
#endif
    }
#endif
    ...
    part.output       = outputR;
    part.num_tuples   = (uint32_t)args->numR;
    part.total_tuples = (uint32_t)args->totalR;
    part.relidx       = 0;
...
#ifdef ITT_NOTIFS
    ocall_itt_event_end(3);
    ocall_itt_event_start(4);
#endif
    ...
    part.output       = outputS;
    part.num_tuples   = (uint32_t)args->numS;
    part.total_tuples = (uint32_t)args->totalS;
    part.relidx       = 1;
...
//                logger(ENCLAVE, "Adding to skew_queue= R:%d, S:%d", ntupR, ntupS);
        ...
        logger(DBG, "Pass-2: # partitioning tasks = %d", part_queue->count);
#ifdef SKEW_HANDLING
        logger(DBG, "Pass-2: # skew tasks         = %d", skew_queue->count);
#endif
        ocall_stopTimer(&args->pass1);
        ocall_startTimer(&args->pass2);
#ifdef ITT_NOTIFS
        ocall_itt_event_end(1);
        ocall_itt_event_end(4);
        ocall_itt_event_start(2);
#endif

#ifdef PCM_COUNT
        ocall_get_system_counter_state("Partition - 1st pass", 0);
        ocall_set_system_counter_state("Partition - 2nd pass");
#endif
    }
...
//        logger(DBG, "Got skew task = R: %d, S: %d",
//                 (*args->skewtask)->relR.num_tuples,
//                 (*args->skewtask)->relS.num_tuples);

        int32_t numperthr = (uint32_t)((*args->skewtask)->relR.num_tuples / args->nthreads);
        ...
        part.hist         = args->histR;
        part.output       = outputR;
        part.num_tuples   = (uint32_t) ((my_tid == (args->nthreads - 1)) ?
                                        ((*args->skewtask)->relR.num_tuples - my_tid * numperthr)
                                                             : numperthr);
        part.total_tuples = (uint32_t)(*args->skewtask)->relR.num_tuples;
        part.relidx       = 2; /* meaning this is pass-2, no syncstats */
        parallel_radix_partition(&part);

        numperthr = (uint32_t)((*args->skewtask)->relS.num_tuples / args->nthreads);
        /* 2. partitioning for relation S */
        ...
        part.num_tuples   = (uint32_t)((my_tid == (args->nthreads - 1)) ?
                                       ((*args->skewtask)->relS.num_tuples - my_tid * numperthr)
                                                                        : numperthr);
        part.total_tuples = (uint32_t)(*args->skewtask)->relS.num_tuples;
        part.relidx       = 2; /* meaning this is pass-2, no syncstats */
        parallel_radix_partition(&part);
        ...
        if(my_tid == 0) {
            const int THR1 = (int) THRESHOLD1((unsigned long) args->nthreads);
...
//                    logger(DBG, "Large join task = R: %d, S: %d", ntupR, ntupS);
...
#ifdef ITT_NOTIFS
        ocall_itt_event_end(2);
#endif
#ifdef PCM_COUNT
        ocall_get_system_counter_state("Partition - 2nd pass", 0);
#endif
    }
#endif
    ...
    if(my_tid == 0)
    {
        logger(DBG, "Number of join tasks = %d", join_queue->count);
    }
    ...
    if (args->join_function == NULL) {
        logger(INFO, "No JOIN function.");
    } else {
        while((task = task_queue_get_atomic(join_queue))){
            /* do the actual join. join method differs for different algorithms,
               ...
               prefetching  */
            results += args->join_function(&task->relR, &task->relS, &task->tmpR, &output, args->materialize);
...
//#endif
    ...
    joinresult->throughput   = (double)(relR->num_tuples + relS->num_tuples) / (double) (args[0].end - args[0].start);
#ifdef SYNCSTATS
...
//#endif
    ...
    for( i=0; i < (1<<NUM_RADIX_BITS); i++ ){
        ...
        if(R_count_per_cluster[i] > 0 && S_count_per_cluster[i] > 0){
            ...
            result += bucket_chaining_join(&tmpR, &tmpS, NULL, nullptr, config->MATERIALIZE);
        }
        else {
            r += (int)R_count_per_cluster[i];
            s += (int)S_count_per_cluster[i];
